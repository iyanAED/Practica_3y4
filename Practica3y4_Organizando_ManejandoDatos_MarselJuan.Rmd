...
```{r}
load("C:/Users/iyana/Desktop/MASTER CIENCIA DE DATOS/AED/Practica_3y4/data/pew_religion.Rdata")

library(tidyr)
library(tidyverse)

pew_religion_tidy <- gather(pew_religion, key = 'income', value = 'freq', 2:11)
...
```{r}
raw_tidy <- pivot_longer(raw,names_to = 'weeks', values_to = 'rank', cols = 6:length(colnames(raw)), names_prefix = 'wk')
...
tb_clean <- gather(raw, key = 'sex_age', value = 'cases', 3:18)%>%
...
```{r}
weather_tidy <- raw %>%
  pivot_longer(cols = starts_with("d"), names_to = "dia", names_prefix = "d", values_to = "values", values_drop_na = TRUE) %>%
  mutate(dia = as.integer(dia)) %>%
  pivot_wider(id_cols  = c(id, year, month, dia), names_from  = element, values_from = values) %>%
  mutate(fecha = as.Date(sprintf("%d-%02d-%02d", year, month, dia))) %>%
  select(id, fecha, tmin, tmax, everything(), -year, -month, -dia)
